...
*Knowledge graph* Sirah Nabawiyah dibangun dari hasil prediksi NER model pemenang (IndoBERT *uncased* dengan *augmentation*) terhadap seluruh korpus, lalu disimpan di Neo4j. Graf hasil konstruksi memuat 1.177 *node* entitas, yaitu 901 *Person*, 167 *Time*, 74 *Location*, dan 35 *Event*, ditambah 15 *node* *Period* sebagai penanda babak kronologis Sirah. Antar *node* terbentuk 705 relasi, yaitu 312 `KELUARGA`, 237 `INVOLVED_IN` (keterlibatan tokoh pada peristiwa), 47 `OCCURRED_AT` (lokasi peristiwa), 47 `OCCURRED_ON` (waktu peristiwa), 33 `SAHABAT`, 17 `PRECEDES` (urutan kronologis antar peristiwa), dan 12 `MUSUH`, ditambah 35 relasi `IN_PERIOD` yang menautkan tiap peristiwa ke periodenya. Setiap relasi menyimpan metadata penelusuran (*provenance*) berupa potongan bukti, nomor halaman, dan penanda *chunk*, sehingga setiap fakta pada graf dapat dilacak balik ke teks sumber. Contoh potongan *knowledge graph* hasil konstruksi ditunjukkan pada Gambar 4.16, yaitu sub-graf salah satu peristiwa (Perang Badr) beserta tokoh, lokasi, dan waktu yang tertaut, sebagai gambaran bentuk graf secara umum.
...
Dominasi sentralitas Muhammad juga terlihat langsung ketika jaringan ego-nya disandingkan dengan tokoh lain. Gambar 4.17 membandingkan jaringan ego Nabi Muhammad dengan jaringan ego Abu Bakar, yaitu seluruh entitas yang terhubung langsung dengan masing-masing tokoh. Simpul Muhammad menautkan jauh lebih banyak tokoh dan peristiwa (sekitar 108 tetangga langsung) daripada Abu Bakar yang jauh lebih sedikit, sehingga selisih *degree centrality* pada Tabel 4.19 (Muhammad 0,7941 berbanding Abu Bakar 0,5441) terbaca sebagai perbedaan kepadatan graf yang mencolok. Perbandingan visual ini menegaskan bahwa dominasi Muhammad bukan sekadar angka, melainkan pola keterhubungan yang kasat mata.

[SISIPKAN GAMBAR 4.17 - Perbandingan Jaringan Ego: Nabi Muhammad (kiri) vs Abu Bakar (kanan)]
<!-- Dua screenshot Neo4j Browser bersanding, skala sama. Ego Muhammad: `MATCH (m:Person {name:"Muhammad"})-[r]-(n) RETURN m,r,n` . Ego Abu Bakar: `MATCH (m:Person {name:"Abu Bakar"})-[r]-(n) RETURN m,r,n` . Screenshot masing-masing lalu sandingkan. Muhammad ~108 tetangga vs Abu Bakar jauh lebih sedikit. -->

### 4.5.3 Komunitas Tokoh

Deteksi komunitas (G3) dengan algoritma Louvain menghasilkan 8 komunitas dengan modularitas Q = 0,2831, nilai yang menunjukkan struktur kelompok yang masih terlihat namun tidak setajam graf yang lebih longgar, wajar mengingat inti peserta peristiwa saling terhubung rapat sehingga batas antar-kelompok melembut. Dua komunitas terbesar mendominasi: komunitas lingkar Muslim inti (66 anggota; tokoh utama Muhammad, Ali, Umar, Abu Bakar, Aisyah, Utsman) dan sebuah komunitas campuran (47 anggota) yang menautkan tokoh oposisi Quraisy (Abu Jahal, Abu Sufyan) dengan sejumlah tokoh Muslim yang banyak terlibat peperangan (Hamzah bin Abdul Muththalib, Zaid bin Haritsah). Percampuran ini sendiri menjadi tanda bahwa pada modularitas serendah ini pemisahan antar-faksi belum tajam, sejalan dengan padatnya jaringan yang dibahas pada Tabel 4.18. Pengelompokan bersifat cukup stabil terhadap pilihan algoritma pada tingkat sedang, ditunjukkan oleh kesepakatan antara Louvain dan *greedy modularity* (*Adjusted Rand Index* 0,47). Karena Neo4j Browser mewarnai simpul menurut labelnya dan bukan menurut komunitas, salah satu komunitas terbesar, yaitu lingkar Muslim inti, ditampilkan sebagai sub-graf tersendiri pada Gambar 4.18 untuk memperlihatkan kerapatan hubungan di dalam satu komunitas.

[SISIPKAN GAMBAR 4.18 - Sub-graf Komunitas Lingkar Muslim Inti pada Neo4j Browser]
<!-- Screenshot Neo4j Browser (tanpa GDS). Neo4j mewarnai per-label, bukan komunitas; pewarnaan komunitas butuh plugin GDS. Sebagai gantinya tampilkan anggota inti satu komunitas + relasi antar-mereka. Query:
MATCH (p:Person)-[r]-(q:Person)
WHERE p.name IN ['Muhammad','Abu Bakar','Umar bin Al-Khaththab','Utsman bin Affan','Ali bin Abu Thalib','Aisyah']
  AND q.name IN ['Muhammad','Abu Bakar','Umar bin Al-Khaththab','Utsman bin Affan','Ali bin Abu Thalib','Aisyah']
RETURN p,r,q
-- lalu screenshot. -->
