...
| Pencocokan String | jellyfish | Jaro-Winkler | Menyatukan variasi penulisan nama entitas (normalisasi alias) |
...
Kode Semu 3.3 Preprocessing Teks Hasil OCR Hasil akhir dari proses ini berupa dataset bersih yang memuat kolom judul_bab, judul_sub_bab, halaman, dan teks_clean. Keluaran tahap ini tidak hanya berfungsi untuk
...
Sebagai gambaran penerapan, sub-bab "Kekuasaan di Berbagai Penjuru Arab" pada halaman 53–54 yang panjang teksnya melebihi 1500 karakter dipecah menjadi dua chunk. Kalimat terakhir chunk pertama, yaitu "Sehingga adakalanya jika seorang pemimpin murka, sekian ribu mata pedang akan ikut berbicara tanpa perlu bertanya apa yang membuat pemimpin kabilah itu murka.", diulang sebagai kalimat pertama chunk kedua sehingga berperan sebagai overlap. Dengan mekanisme ini, konteks pada batas antar-chunk tetap terjaga meskipun teks dipisah, sedangkan metadata bab, sub-bab, dan halaman tetap dipertahankan pada setiap chunk.
...
Proses koreksi manual mencakup penambahan entitas yang belum terdeteksi, penghapusan kandidat entitas yang tidak sesuai, serta perbaikan batas awal dan akhir entitas. Tahap ini penting untuk memastikan bahwa data anotasi yang digunakan pada proses pelatihan dan evaluasi memiliki kualitas yang lebih terkontrol. Koreksi manual diperlukan karena pelabelan semi-otomatis berbasis kamus, regex, dan hasil OCR dapat menghasilkan kesalahan sistematis, yaitu entitas yang terlewat, batas entitas yang salah, kesalahan tipe (terutama nama yang dapat berupa lokasi sekaligus peristiwa), entitas palsu, serta nama yang terpecah akibat artefak OCR. Beberapa contoh perbaikan pada penelitian ini meliputi kata "India" yang semula tidak teranotasi menjadi berlabel LOCATION, angka "610" pada "610 M" yang dimasukkan sebagai bagian entitas TIME, frasa "peperangan Badr" yang semula berlabel LOCATION diperbaiki menjadi EVENT, penyebutan pada rujukan kitab seperti "bab Ghazwah Dzatu Qarad" yang dibatalkan menjadi bukan entitas, serta bentuk "Mush ab" yang disambung menjadi "Mush'ab". Koreksi mengacu pada pedoman anotasi yang mengunci keputusan pelabelan dan diterapkan pada satu sumber kebenaran di tingkat rentang (span). Koreksi dilakukan oleh peneliti sebagai anotator tunggal, sehingga tidak terdapat anotator kedua maupun pengukuran kesepakatan antar-anotator; keterbatasan ini dinyatakan secara eksplisit. Contoh daftar label beserta keterangannya ditunjukkan pada Tabel 3.8. **Tabel 3.8 Skema Label Entitas**
...
Anotasi tingkat rentang (span) selanjutnya dikonversi ke dalam format token-per-baris menggunakan skema penandaan Begin-Inside-Outside (BIO). Format ini diperlukan karena model NER memproses data sebagai tugas sequence labeling. Teks pada setiap chunk ditokenisasi berdasarkan spasi, sekaligus dicatat posisi karakter dari setiap token. Setelah itu, setiap token diberi label sesuai posisinya terhadap rentang entitas. Token awal dari suatu entitas diberi tag B-LABEL, token lanjutan dari entitas yang sama diberi tag I-LABEL, sedangkan token yang bukan bagian dari entitas diberi tag O. Skema BIO digunakan karena merupakan salah satu formulasi standar dalam tugas NER sebagai sequence labeling, sebagaimana dijelaskan pada Bab 2 Subbab 2.3.1. Implementasi konversi BIO ditunjukkan pada **Kode Semu 3.6**
...
Nilai hyperparameter tersebut tidak ditetapkan secara sembarang, melainkan diadopsi sebagai nilai awal dari penelitian acuan yang menangani tugas sejenis, yaitu NER berbasis IndoBERT dengan iterative self-training pada bahasa Indonesia berdaya rendah (Ariyanto et al., 2025), kemudian diterapkan dan diuji kembali pada dataset Sirah Nabawiyah. Learning rate 2e-5 dipilih karena merupakan nilai yang umum efektif untuk model berbasis transformer dan cukup kecil untuk menjaga kestabilan pelatihan. Batch size 16 dipilih untuk menyesuaikan pemakaian memori GPU tanpa mengorbankan efisiensi pelatihan. Pelatihan dilakukan selama 10 epoch pada setiap iterasi dengan mekanisme early stopping untuk mencegah overfitting, disertai weight decay 0,01 untuk memperbaiki generalisasi. Ambang confidence 0,9 digunakan pada penyaringan pseudo-label; pada penelitian acuan, ambang 0,7, 0,8, dan 0,9 telah dibandingkan dan nilai 0,9 memberikan hasil terbaik karena hanya menerima prediksi berkepercayaan tinggi, dengan confidence sebuah prediksi dihitung sebagai rata-rata skor keyakinan seluruh token entitas dalam kalimat. Adapun jumlah iterasi self-training dibatasi maksimal enam iterasi sebagai titik henti praktis karena peningkatan F1 antar-iterasi sudah landai; batasan ini merupakan penyesuaian pada penelitian ini dan bukan berasal dari penelitian acuan.
...
Tahap konstruksi knowledge graph menyusun struktur graf dari daftar entitas hasil ekstraksi melalui empat bagian yang berurutan: penyatuan variasi nama entitas (normalisasi alias) agar satu entitas tidak terpecah menjadi banyak node, pembentukan relasi antar entitas menjadi pasangan node-edge, periodisasi peristiwa yang menambahkan dimensi waktu berupa atribut periode dan relasi urutan kronologis antar peristiwa, lalu pemuatan node dan edge tersebut ke basis data graf Neo4j. Diagram alir keseluruhan tahap konstruksi knowledge graph ditunjukkan pada Gambar 3.7.

Gambar 3.7 Diagram Alir Tahapan Konstruksi Knowledge Graph

Agar keterhubungan antar-bagian tidak hanya terlihat secara konseptual, alur konstruksi ditelusuri melalui satu contoh data yang sama. Dari sebuah chunk pada halaman 165–168, kalimat "Tatkala Abu Jahal mengajaknya pergi saat Perang Badr…" ditokenisasi dan diberi label BIO, sehingga "Abu Jahal" berlabel B-PERSON dan I-PERSON serta "Perang Badr" berlabel B-EVENT dan I-EVENT. Hasil ekstraksi menghasilkan dua entitas, yaitu Abu Jahal bertipe Person dan Perang Badr bertipe Event. Kedua nama dinormalisasi ke bentuk kanonik melalui normalisasi alias. Karena kedua entitas berada dalam satu kalimat, keduanya menjadi pasangan kandidat relasi, lalu pasangan Person dan Event dipetakan menjadi relasi INVOLVED_IN dengan bobot sesuai kedekatan. Selanjutnya dibentuk simpul Abu Jahal dan simpul Perang Badr beserta sisi INVOLVED_IN di antaranya, kemudian disimpan ke Neo4j. Relasi ini tetap dapat ditelusuri kembali ke halaman sumber melalui properti bukti (evidence). Contoh yang sama digunakan sebagai benang merah pada subbab-subbab berikut, mulai dari normalisasi alias hingga pemuatan graf ke Neo4j. Agar setiap tahap tergambar secara konkret, penelusuran contoh tersebut dirangkum pada Tabel 3.13 dalam bentuk masukan, proses, dan keluaran per tahap, sedangkan bentuk akhir subgrafnya diilustrasikan pada Gambar 3.9.

**Tabel 3.13 Penelusuran Contoh Berjalan pada Konstruksi Knowledge Graph**

| Tahap (Subbab) | Masukan pada contoh | Proses | Keluaran |
|---|---|---|---|
| Ekstraksi entitas (3.6) | Token "Abu Jahal" → B-PERSON, I-PERSON; "Perang Badr" → B-EVENT, I-EVENT | Penggabungan token berlabel B- dan I- yang berurutan menjadi entitas | Dua entitas: Abu Jahal (Person), Perang Badr (Event) |
| Normalisasi alias (3.7.1) | Abu Jahal; Perang Badr (dengan variasi "Badr") | Pemetaan variasi nama ke satu bentuk kanonik | Simpul kanonik: Abu Jahal; Perang Badr |
| Pembentukan relasi (3.7.2) | Pasangan (Abu Jahal, Perang Badr) yang berada dalam satu kalimat | Pasangan Person–Event dipetakan ke relasi INVOLVED_IN dengan bobot menurut kedekatan | Sisi: Abu Jahal —INVOLVED_IN→ Perang Badr |
| Periodisasi peristiwa (3.7.3) | Simpul Event Perang Badr | Pemetaan peristiwa ke periode naratif | Atribut periode "Perang Badr & Dampaknya" pada simpul Event |
| Konstruksi graf di Neo4j (3.7.4) | Simpul, sisi, dan atribut di atas | Pemuatan node dan edge ke basis data graf beserta properti bukti | Subgraf tersimpan; properti evidence menunjuk halaman 165-168 |

Gambar 3.9 Ilustrasi Subgraf Hasil Contoh Berjalan. Subgraf akhir dari contoh ini terdiri atas dua simpul dan satu sisi berarah, yaitu simpul Abu Jahal bertipe Person dan simpul Perang Badr bertipe Event yang dihubungkan oleh sisi INVOLVED_IN. Simpul Perang Badr membawa atribut periode "Perang Badr & Dampaknya", sedangkan sisi INVOLVED_IN membawa properti bukti berupa halaman sumber 165-168 sehingga relasi dapat ditelusuri kembali ke teks aslinya.

### 3.7.1 Normalisasi Alias
...
Gambar 3.10 Diagram Alir Tahapan Pembentukan Relasi **Tabel 3.15 Rancangan Tipe Relasi Inti**
...
daftar edge (edges.csv) yang masing-masing dilengkapi atribut dan provenance. Dengan demikian, keluaran tahap ini tidak hanya merepresentasikan entitas sebagai titik dalam graf, tetapi juga memuat keterhubungan bermakna antara tokoh, peristiwa, lokasi, dan waktu yang siap dimuat ke dalam basis data graf. Atribut node dan edge dijelaskan pada Tabel 3.16 dan Tabel 3.17. **Tabel 3.16 Atribut Node**
...
Bobot relasi dihitung dari kombinasi skor kedekatan (proximity) entitas dalam teks dan skor periode (kesesuaian relasi dengan periode peristiwa, lihat subbab 3.7.3). Contoh hasil satu relasi adalah (Person: Abu Bakar) - [INVOLVED_IN] → (Event: Hijrah ke Madinah) dengan evidence berupa cuplikan kalimat sumber dan weight tertentu. Beberapa contoh edge nyata untuk tiap tipe relasi ditunjukkan pada Tabel 3.18. Keluaran tahap ini berupa nodes.csv dan edges.csv. **Tabel 3.18 Contoh Edge Hasil Pembentukan Relasi**
