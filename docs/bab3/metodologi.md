...
Evaluasi fungsional graf memverifikasi bahwa struktur graf dapat mendukung penelusuran relasional. Pendekatan yang digunakan adalah pengujian berbasis skenario kueri (*query-based functional testing*) karena tujuan utama graf adalah menjawab kebutuhan penelusuran berbasis hubungan. Disusun sejumlah skenario kueri Cypher yang mewakili kebutuhan penelusuran pada Sirah, mencakup kueri berbasis tokoh, lokasi, waktu, serta kueri *multi-hop*. Rancangan skenario kueri ditunjukkan pada Tabel 3.20.
...
Setiap kueri dijalankan pada graf, lalu hasilnya diverifikasi melalui beberapa kriteria: kueri dapat dieksekusi tanpa galat, hasil tidak kosong, hasil sesuai dengan fakta pada teks sumber melalui validasi manual, dan hasil dapat dilacak kembali ke dokumen sumber melalui metadata *provenance* (`evidence`, `halaman`, `chunk_id`). Keluaran berupa tabel hasil eksekusi kueri (status keberhasilan dan jumlah hasil), contoh hasil kueri sebagai ilustrasi, serta penilaian kelayakan graf dalam mendukung penelusuran informasi relasional pada Sirah Nabawiyah. Hasil dan pembahasannya disajikan pada Bab 4.
